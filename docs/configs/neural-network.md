...
Topology changes propagate through the codebase via `NeuralNetwork.from_genome()` (`nn/network.py`) and `create_population()` (`ga/engine.py`). The genome encoding is flat: `[hidden_1_weights, hidden_1_bias, hidden_2_weights, hidden_2_bias, ..., output_weights, output_bias]`. For a 1-layer brain with `hidden_layer_size=6` and 4 input features, the genome is `(6×4) + 6 + 6 + 1 = 37` values.
...
Deeper networks don't guarantee better brains — the GA has to find weights that work, and a deeper search space is harder to explore. Useful as a teaching example of depth vs. width in neural networks.
