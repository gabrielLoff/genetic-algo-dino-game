import json
import os
from dataclasses import dataclass

PARAM_SPECS = [
    ("population_size", 100, 10, 1000, "Genetic Algorithm", "Population Size", int,
     "Number of brains competing each generation", None),
    ("mutation_rate", 0.1, 0.0, 1.0, "Genetic Algorithm", "Mutation Rate", float,
     "Probability each gene is randomly altered", None),
    ("mutation_strength", 0.2, 0.01, 2.0, "Genetic Algorithm", "Mutation Strength", float,
     "Magnitude of random changes to genes", None),
    ("mutation_adaptation", "none", None, None, "Genetic Algorithm", "Mutation Adaptation", str,
     "How mutation strength adapts: none, linear_decay, diversity_driven",
     ["none", "linear_decay", "diversity_driven"]),
    ("mutation_strength_floor", 0.05, 0.0, 0.5, "Genetic Algorithm", "Mutation Floor", float,
     "Minimum effective mutation strength when adaptation is active", None),
    ("mutation_strength_cap", 1.0, 0.0, 5.0, "Genetic Algorithm", "Mutation Cap", float,
     "Maximum effective mutation strength for diversity-driven adaptation", None),
    ("tournament_size_percent", 0.1, 0.01, 1.0, "Genetic Algorithm", "Tournament Size %", float,
     "Fraction of population competing to be a parent", None),
    ("elitism_rate", 0.05, 0.0, 0.5, "Genetic Algorithm", "Elitism Rate", float,
     "Fraction of top brains kept unchanged each generation", None),
    ("fitness_function", "survival_clearance", None, None, "Genetic Algorithm", "Fitness Function", str,
     "Strategy for scoring brains: survival, clearance, near-miss, efficiency",
     ["survival_only", "survival_clearance", "near_miss", "efficiency"]),
    ("crossover_operator", "uniform", None, None, "Genetic Algorithm", "Crossover Operator", str,
     "How parent genomes combine: uniform, single_point, two_point",
     ["uniform", "single_point", "two_point"]),
    ("max_generations", 50, 1, 500, "Genetic Algorithm", "Max Generations", int,
     "Stop evolution after this many generations", None),
    ("plateau_generations", 10, 1, 100, "Genetic Algorithm", "Plateau Gens", int,
     "Stop if best fitness doesn't improve for this many gens", None),
    ("master_seed", None, None, None, "Genetic Algorithm", "Master Seed", None,
     "Fixed seed for reproducible evolution; Random if None", None),
    ("narrated_mode", False, False, True, "Genetic Algorithm", "Narrated Mode", None,
     "Print plain-English generation summaries for presentations", None),
    ("curriculum_mode", False, False, True, "Genetic Algorithm", "Curriculum Mode", None,
     "Scale obstacle difficulty up as fitness improves", None),
    ("hidden_layer_size", 6, 1, 100, "Neural Network", "Hidden Layer Size", int,
     "Number of neurons in each hidden layer of the brain", None),
    ("num_hidden_layers", 1, 1, 3, "Neural Network", "Hidden Layers", int,
     "Number of hidden layers in the brain (1-3)", None),
    ("output_size", 2, 1, 3, "Neural Network", "Output Size", int,
     "Number of output neurons: 1 (jump only), 2 (jump + crouch)", None),
    ("game_speed_initial", 400, 50, 2000, "Game", "Initial Speed", float,
     "Starting scroll speed (pixels/sec)", None),
    ("game_speed_max", 1000, 100, 5000, "Game", "Max Speed", float,
     "Maximum scroll speed the game reaches", None),
    ("game_speed_increment", 2, 0.1, 50, "Game", "Speed Increment", float,
     "How much speed increases per second", None),
    ("obstacle_min_gap", 200, 50, 1000, "Game", "Min Obstacle Gap", float,
     "Minimum distance between consecutive obstacles", None),
    ("obstacle_gap_mean", 500, 100, 2000, "Game", "Obstacle Gap Mean", float,
     "Average distance between obstacles (exponential distribution)", None),
    ("obstacle_gap_decay", 0.001, 0.0, 0.1, "Game", "Gap Decay", float,
     "How fast the average gap shrinks over time", None),
    ("pterodactyl_probability", 0.3, 0.0, 1.0, "Game", "Pterodactyl Prob.", float,
     "Probability a spawned obstacle is a pterodactyl instead of a cactus", None),
    ("diversity_warning_threshold", 0.05, 0.0, 1.0, "Game", "Diversity Warn", float,
     "Triggers low-diversity warning and escalates mutation strength when adaptation is diversity-driven", None),
    ("time_cap_seconds", 30, 1, 300, "Game", "Time Cap (s)", float,
     "Maximum seconds a brain can survive in a single run", None),
    ("jump_cooldown_frames", 5, 1, 30, "Game", "Jump Cooldown", int,
     "Frames a brain must wait between consecutive jumps", None),
    ("collision_inset", 0.15, 0.0, 0.5, "Game", "Collision Inset", float,
     "How much smaller the collision box is than the sprite", None),
    ("dino_gravity", 2000, 100, 5000, "Game", "Gravity", float,
     "Downward acceleration of the dino (pixels/sec^2)", None),
    ("dino_max_jump_velocity", -600, -2000, -10, "Game", "Max Jump Velocity", float,
     "Upward velocity at full jump (negative = upward)", None),
    ("ground_height", 80, 40, 200, "Game", "Ground Height", int,
     "Height of the ground area at the bottom of the screen", None),
    ("window_width", 1024, 400, 2560, "Game", "Window Width", int,
     "Game window width in pixels", None),
    ("window_height", 640, 200, 1440, "Game", "Window Height", int,
     "Game window height in pixels", None),
    ("obstacle_seed", None, None, None, "Game", "Obstacle Seed", None,
     "Fixed seed for obstacle layout; Random if None", None),
    ("ghost_mode", "off", None, None, "Game", "Ghost Mode", str,
     "Show ghost brains in replay: off, worst, random, top",
     ["off", "worst", "random", "top"]),
    ("ghost_count", 3, 1, 10, "Game", "Ghost Count", int,
     "Number of ghost brains when mode is top", None),
    ("fullscreen", False, False, True, "Game", "Fullscreen", None,
     "Run in borderless fullscreen for presentations", None),
]

DEFAULT_CONFIG = {
    name: default for name, default, _min, _max, _group, _label, _type, _desc, _options in PARAM_SPECS
}


@dataclass
class Config:
    population_size: int = 100
    hidden_layer_size: int = 6
    num_hidden_layers: int = 1
    output_size: int = 2
    mutation_rate: float = 0.1
    mutation_strength: float = 0.2
    mutation_adaptation: str = "none"
    mutation_strength_floor: float = 0.05
    mutation_strength_cap: float = 1.0
    tournament_size_percent: float = 0.1
    elitism_rate: float = 0.05
    fitness_function: str = "survival_clearance"
    crossover_operator: str = "uniform"
    game_speed_initial: float = 400
    game_speed_max: float = 1000
    game_speed_increment: float = 2
    obstacle_min_gap: float = 200
    obstacle_gap_mean: float = 500
    obstacle_gap_decay: float = 0.001
    pterodactyl_probability: float = 0.3
    diversity_warning_threshold: float = 0.05
    time_cap_seconds: float = 30
    jump_cooldown_frames: int = 5
    collision_inset: float = 0.15
    dino_gravity: float = 2000
    dino_max_jump_velocity: float = -600
    ground_height: int = 80
    window_width: int = 1024
    window_height: int = 640
    max_generations: int = 50
    plateau_generations: int = 10
    master_seed: int | None = None
    obstacle_seed: int | None = None
    ghost_mode: str = "off"
    ghost_count: int = 3
    fullscreen: bool = False
    narrated_mode: bool = False
    curriculum_mode: bool = False
    curriculum_tier: int = 0

    @property
    def ground_y(self):
        return self.window_height - self.ground_height


def load_config(path):
    overrides = {}
    if os.path.exists(path):
        with open(path) as f:
            overrides = json.load(f)
    return Config(**overrides)
